...
int argument_is_easy(machine_t *machine, int code[4], \
...
void write_in_good_reg(machine_t *machine, int args[3], champion_t *champion)
{
    ...
    if (sum == 0)
        ...
        champion->carry = 0;
    address = \
    (champion->pc + sum % IDX_MOD) % MEM_SIZE;
...
void exec_ldi(machine_t *machine, champion_t *champion)
{
    ...
    args[0] = argument_is_easy(machine, code, binary, champion);
    code[2] += 2;
    code[2] = code[2] % MEM_SIZE;
    code[3] += 2;
    args[1] = argument_is_easy(machine, code, binary, champion);
    code[2] += 2;
    code[2] = code[2] % MEM_SIZE;
    args[2] = get_good_number(machine, code[2], T_REG) - 1;
    write_in_good_reg(machine, args, champion);
